...
func addCreateFlags(fs *pflag.FlagSet, cf *run.CreateFlags) {
	...
	fs.StringVar(&cf.MemoryString, "memory", constants.VM_DEFAULT_MEMORY, "VM RAM in MiB")
	fs.StringVarP(&cf.SizeString, "size", "s", constants.VM_DEFAULT_SIZE, "VM filesystem size, for example 5GB or 2048MB")
